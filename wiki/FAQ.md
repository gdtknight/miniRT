...
**A**: 네, 다중 광원을 지원합니다. L 요소를 여러 개 추가할 수 있으며 최대 MAX_LIGHTS개까지 가능합니다.
...
1. **객체 수**: 50개 이상이면 초기 BVH 빌드에 시간이 걸립니다.
2. **해상도**: 1440x900 고정 해상도입니다.
3. **Soft Shadow**: 인터랙션 중에는 자동으로 1 sample로 축소됩니다 (디바운싱).
...
**사용 방법**:
- 별도 설정 불필요 (기본 활성화)
- 카메라 조작 시 자동 작동 (W/X/A/D/Q/Z/E/C)
