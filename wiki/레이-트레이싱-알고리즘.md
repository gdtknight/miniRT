...
원뿔: 꼭짓점 V, 축 방향 A, 반지름 r (밑면), 높이 h

원뿔 교차는 원기둥과 유사하게 3부분으로 나뉩니다:
1. **원뿔면 (Conical Surface)**: 축 방향으로 반지름이 선형으로 변하는 이차곡면
2. **밑면 캡 (Bottom Cap)**: 평면 교차 후 반지름 검사

원뿔면의 교차 검사는 원기둥과 같은 이차방정식 구조이지만, 반지름이 높이에 따라 선형으로 변하므로 계수가 달라집니다. 높이 제한과 밑면 캡 처리는 원기둥과 동일한 방식으로 처리됩니다.
...
> **v2.2.0부터 Plane은 BVH 트리에서 제외됩니다.** Plane의 AABB가 무한대이므로 BVH 트리 품질을 저하시킵니다. Plane은 `t_plane_refs`에 별도 저장되며 BVH 순회 후 선형 테스트합니다.
...
5. **리프 노드 생성**: 2개 이하의 객체는 리프 노드로 저장
...
> **최적화**: 실제 구현에서는 `inv_dir` (1.0 / ray.dir)이 광선 구조체(`t_ray`)에 사전 계산되어 저장됩니다. 이를 통해 AABB 교차 검사 시 매번 나눗셈을 수행하지 않고 곱셈으로 대체하여 성능을 향상시킵니다.
...
- v2.1.0부터 기본 활성화
- 별도 설정 불필요
- bounded 오브젝트(구, 원기둥, 원뿔) 지원. 평면은 BVH에서 분리되어 별도 테스트
- Shadow BVH any-hit 최적화: 그림자 광선은 `bvh_any_hit()`으로 첫 번째 차폐에서 즉시 반환 (closest-hit 불필요)
