...
MiniLibX 디스플레이 초기화, 이벤트 핸들링, HUD 오버레이, 키가이드를 담당하는 모듈입니다.
...
| 파일 | 역할 |
|------|------|
| `display_init.c` | MiniLibX 초기화 (`mlx_init`, `mlx_new_window`, `mlx_new_image`) |
| `display_destroy.c` | MiniLibX 정리 (`mlx_destroy_image`, `mlx_destroy_window`) |
| `display_events.c` | `handle_key`, `close_window` -- 키 이벤트 분기 및 종료 처리 |
| `display_pixel.c` | 픽셀 쓰기 (`put_pixel_to_buffer`) 및 형식 변환 |

### Input (`src/input/`)

| 파일 | 역할 |
|------|------|
| `input_dispatch.c` | 키 입력 분기(dispatch) (`handle_hud_keys`, `handle_camera_keys`, `handle_transform_keys`) |
...
| 항목 | macOS | Linux |
|------|-------|-------|
| 키 코드 | `includes/input.h`에 `#ifdef __APPLE__` 섹션 | `#ifdef __linux__` 섹션 |
